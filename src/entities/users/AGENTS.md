...
- Sign and verify access tokens with `JWT_SECRET_KEY`; sign and verify refresh tokens with `JWT_REFRESH_SECRET_KEY`. Each pair includes a random session ID and is accepted only while its Redis session maps to the signed user ID. Password resets and changes, disabling, and deletion invalidate every session for the affected user.
