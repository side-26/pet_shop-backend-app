...
- `GET /landing/products` — enabled products filtered by category, sub-category, brand, and an inclusive raw-price range. It supports `most-valued`, `less-valued`, `most-sales`, and `less-sales`, and returns `{ result, pagination }`.
...
- Product-list queries always require `isEnable: true`; their sort modes use title then ID as deterministic tie-breakers.
