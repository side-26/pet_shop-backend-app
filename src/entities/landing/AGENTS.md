...
- `GET /landing/products/discounted?limit=COUNT` — enabled products ordered by discount percentage, with the main image thumbnail, source price, discount percentage, response-only calculated discount amount, and minimum final price across weights; `limit` defaults to four and is capped at 100.
- `GET /landing/pet-types/all` — every enabled pet type, ordered by creation date, each with its main image, thumbnail, and summary.
- `GET /landing/products/popular` — up to four enabled products, ordered by sales volume, with the main image thumbnail, slug, source price, discount percentage, response-only final discounted price, and minimum final price across weights.
- `GET /landing/products` — enabled products filtered by comma-separated category, sub-category, and brand IDs, an inclusive raw-price range, and enabled status. It supports `most-valued`, `less-valued`, `most-sales`, and `less-sales`, and returns `{ result, pagination, filters, sort }` with each product's minimum final price; `isEnable=false` returns no public records.
...
- `GET /landing/products/featured` — up to four distinct enabled products in priority order: most purchased, most discounted, cheapest, then most wishlisted, each with its minimum final price across weights. A product selected for an earlier category is excluded from later categories.
