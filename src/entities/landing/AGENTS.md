...
- `GET /landing/products` — enabled products filtered by comma-separated category, sub-category, and brand IDs, an inclusive raw-price range, and availability. It supports `most-valued`, `less-valued`, `most-sales`, and `less-sales`, and returns `{ result, pagination, filters, sort }`.
...
- Product-list queries always require `isEnable: true`; their sort modes use title then ID as deterministic tie-breakers. Facet counts use disjunctive filtering: each facet is calculated with its own selected value excluded, while all other selected values remain applied.
