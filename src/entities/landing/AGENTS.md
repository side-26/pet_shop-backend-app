...
- `GET /landing/products` — enabled products filtered by comma-separated category, sub-category, and brand IDs, an inclusive raw-price range, availability, and enabled status. It supports `most-valued`, `less-valued`, `most-sales`, and `less-sales`, and returns `{ result, pagination, filters, sort }`; `isEnable=false` returns no public records.
...
- Product-list queries always require `isEnable: true`; their final `isEnable` boolean facet advertises the enabled status while preserving this public-catalog rule, so `isEnable=false` has a zero count and returns no public records. Sort modes use title then ID as deterministic tie-breakers. Facet counts use disjunctive filtering: each facet is calculated with its own selected value excluded, while all other selected values remain applied.
