...
- `breeds.service.js` — validates pet-type relationships and owns CRUD, property-definition replacement/read formatting, slug lookup, required main-image upload/replacement, thumbnail generation, status, filtering, pagination, and formatting.
...
- Create and update require a JPEG/JPG, PNG, or WebP `mainImage` multipart file smaller than 1 MB; persist its public URL as `mainImage` and the generated sub-10 KB data URL as `thumbnailImage`.
