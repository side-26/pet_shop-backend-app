...
- `auth/redisAuthSession.store.js` — atomically creates TTL-bounded, user-owned login sessions and removes every indexed session when an account is revoked, disabled, deleted, or its password changes.
